...
const Essays = () => {
  ...
  useEffect(() => {
    // Mock data with full content added
    const mockEssays = [
      {
        id: 1,
  ...
  if (error) return <div className="error">Error loading essays: {error}</div>;
        ...
        <div className="modal-overlay">
          <div className="essay-modal-content">
            ...
            <div className="essay-meta">
              <span className="essay-date">Posted on {new Date(selectedEssay.date).toLocaleDateString()}</span>
              <div className="essay-rating">
                {renderStars(selectedEssay.averageRating)}
                <span>({selectedEssay.reviews?.length || 0} reviews)</span>
              </div>
            </div>
            
            <div 
              className="essay-full-content"
              dangerouslySetInnerHTML={{ __html: selectedEssay.fullContent }}
            />
            
            <div className="essay-reviews">
              <h3>Reviews</h3>
              {selectedEssay.reviews?.length > 0 ? (
                selectedEssay.reviews.map((review, index) => (
                  <div key={index} className="review">
                    <div className="review-header">
                      <span className="reviewer-name">{review.studentName || 'Anonymous'}</span>
            ...
            <div className="modal-actions">
              <button 
                ...
                onClick={() => {
                  closeEssayModal();
                  openReviewModal(selectedEssay.id);
                }}
              >
                Leave Review
              </button>
